    ...
    kumaGauge.prototype = {
    	init: function () {
          ...
          draw : function() {
            ...
            // Draw the gauge
            _this.gauge.gauge = _this.gauge.circularArc(_this.config.centerX, _this.config.centerY, _this.config.radius, 180, 0);
            ...
            _this.gauge.gaugeBackground = _this.gauge.circularArc(_this.config.centerX, _this.config.centerY, _this.config.radius, 180, 0);
            ...
            if (_this.config.showNeedle) {
            	_this.gauge.needle = _this.gauge.rect(_this.config.centerX, _this.config.paddingY - 2, 1, 40);
            	// _this.gauge.needle = _this.gauge.rect(_this.config.centerX, _this.config.paddingY - 2, 1, 40);
            	_this.gauge.needle.attr({
            		'fill' : '#000',
            		'stroke' : 'none',
            		'height' : _this.config.needleHeight
            	});
            	_this.gauge.needle.node.setAttribute('class', 'gauge__needle');
            }

            _this.gauge.needleArc = _this.gauge.circularArc(_this.config.centerX, _this.config.centerY, 40, 180, 0);
            _this.gauge.needleArc.attr({
            	'fill' : '#FFF',//_this.config.background,
            	'stroke' : 'none'
            });
            _this.gauge.centerArc.node.setAttribute('class', 'gauge__needleArc');
            ...
            if (_this.config.valueLabel.display) {
            	// if (_this.config.showNeedle) {
            	// 	_this.gauge.valueLabel = _this.gauge.text(_this.config.centerX, _this.config.centerY - 10, Math.round((_this.config.max - _this.config.min) / 2)); // Math.round((_this.config.max - _this.config.min) / 2)
            	// } else {
            		_this.gauge.valueLabel = _this.gauge.text(_this.config.centerX, _this.config.centerY - 10, _this.config.value + '%');
            	// }
            ...
            //Draw the benchmark
            if(_this.config.benchmark){
            	if(_this.config.benchmark.display){
            		_this.gauge.benchmark = _this.gauge.text(_this.config.centerX, _this.config.paddingY - 25, 'Benchmark ' + _this.config.benchmark.value +'%');
            		_this.gauge.benchmark.attr({
            			'fill' : _this.config.benchmark.fontColor,
            			'font-size' : _this.config.benchmark.fontSize,
            			'font-family' : _this.config.benchmark.fontColor,
            			'font-weight' : _this.config.benchmark.fontWeight
            		});
            		_this.gauge.benchmark.node.setAttribute('class', 'gauge__benchmark');
            	}
            }
            ...
            if (_this.config.title.display) {
            	_this.gauge.title = _this.gauge.text(_this.config.centerX, _this.config.paddingY - 5, _this.config.title.value);
            ...
            if (_this.config.label.display) {
                // Draw the left label
                _this.gauge.leftLabel = _this.gauge.text((_this.config.gaugeWidth / 2) + _this.config.paddingX, _this.config.centerY + 15, _this.config.label.left);
                ...
                _this.gauge.rightLabel = _this.gauge.text((_this.config.width - (_this.config.gaugeWidth / 2)) - _this.config.paddingX,	_this.config.centerY + 15, _this.config.label.right);
            ...
            setTimeout(function() {
            	...
            	if (_this.config.title.display) {
            		_this.gauge.title.attr({
            			'y' : _this.config.paddingY - ((_this.gauge.title.getBBox().height / 2) - 120),
            			'fill-opacity' : 1
            		});
            	}
